...
#if defined(NDEBUG)
#error "Genero cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Genero Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) return 250000 * COIN;
		
	int64_t nSubsidy, nChkCnt;
	...
	if( nHeight > 1 && nHeight <= 5000 ) {
	        ...
	        nSubsidy = 100 * COIN;
	}else if( nHeight > 24000 && nHeight <= 28000 ) {
	        nSubsidy = 130 * COIN;
	}else if( nHeight > 28000 && nHeight <= 54000 ) {
	        nSubsidy = 200 * COIN;
	} else if( nHeight > 54000 && nHeight <= 800000 ) {
		nSubsidy = 100 * COIN;
	} else {
		nChkCnt = (nHeight-795000)/5000;
		if(nChkCnt > 5) {
			nSubsidy = (100/pow(2,5)) * COIN;			
		} else {
			nSubsidy = (100/pow(2,nChkCnt)) * COIN;
		}
	} 
