...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GNROMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GNROMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("GNROMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("genero-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("GNROMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running GNROMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadGNROMiner() exception");
    } catch (...) {
        LogPrintf("ThreadGNROMiner() exception");
    }

    LogPrintf("ThreadGNROMiner exiting\n");
}
